...
pub struct Client {
    lightning: rpc::lightning_client::LightningClient<tonic::codegen::InterceptedService<tonic::transport::Channel, MacaroonInterceptor>>,
    wallet: walletrpc::wallet_kit_client::WalletKitClient<tonic::codegen::InterceptedService<tonic::transport::Channel, MacaroonInterceptor>>,
}

impl Client {
    pub fn lightning(&mut self) -> &mut rpc::lightning_client::LightningClient<tonic::codegen::InterceptedService<tonic::transport::Channel, MacaroonInterceptor>> {
        &mut self.lightning
    }

    pub fn wallet(&mut self) -> &mut walletrpc::wallet_kit_client::WalletKitClient<tonic::codegen::InterceptedService<tonic::transport::Channel, MacaroonInterceptor>> {
        &mut self.wallet
    }
}
...
pub async fn connect<A, CP, MP>(address: A, cert_file: CP, macaroon_file: MP) -> Result<Client, ConnectError> where A: TryInto<tonic::transport::Endpoint> + std::fmt::Debug + ToString, <A as TryInto<tonic::transport::Endpoint>>::Error: std::error::Error + Send + Sync + 'static, CP: AsRef<Path> + Into<PathBuf> + std::fmt::Debug, MP: AsRef<Path> + Into<PathBuf> + std::fmt::Debug {
    ...
    let client = Client {
        lightning: rpc::lightning_client::LightningClient::with_interceptor(conn.clone(), interceptor.clone()),
