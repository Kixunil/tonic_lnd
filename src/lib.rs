...
impl tonic::service::Interceptor for MacaroonInterceptor {
    fn call(&mut self, mut request: tonic::Request<()>) -> Result<tonic::Request<()>, Error> {
        let macaroon: tonic::metadata::MetadataValue =
            self.macaroon.try_into().expect("hex produced non-ascii");
...
mod tls {
    ...
    pub(crate) async fn config(
        ...
        let verifier = Arc::new(CertVerifier::load(path).await?);
        let config = ClientConfig::builder()
            .with_safe_defaults()
            .with_custom_certificate_verifier(verifier)
            .with_no_client_auth();

        // TODO: Do we need to set protocols?
